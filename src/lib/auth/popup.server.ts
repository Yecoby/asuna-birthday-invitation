...
/** The `__Host-` name, also checked when the plain-http localhost variant is in use. */
const LEGACY_SESSION_TOKEN_COOKIE = "__Host-grok-auth.session_token";
...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  if (done) {
    ...
    // Read the token under BOTH possible cookie names: on plain-http localhost
    // the `__Host-` prefix (and `Secure`) is dropped so the browser will store
    // the cookie at all — see the cookie policy in `server.ts`. Reading both
    // keeps the popup completion working in every environment.
    const token = errored
      ? null
      : (readCookie(request, SESSION_TOKEN_COOKIE) ??
        readCookie(request, LEGACY_SESSION_TOKEN_COOKIE));
