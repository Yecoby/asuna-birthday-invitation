...
import { disableSignUp, emailAndPasswordEnabled } from "./email-password";
import { GATE_PROVIDER_ID, gateIdentitySessions } from "./gate-session.server";
import {
  accountDataCookieName as accountDataCookieNameFor,
  dontRememberCookieName as dontRememberCookieNameFor,
  sessionDataCookieName as sessionDataCookieNameFor,
...
/**
 * The origin this server is really being reached on.
 *
 * `BETTER_AUTH_URL` is authoritative once a deployment injects it. Locally it is
 * unset, and `baseURL` below is a DYNAMIC config (resolved per request), so we
 * fall back to the dev-server contract as the single local origin — matching
 * `fallback: "http://localhost:8080"`.
 */
const localOrigin = explicitBaseURL ?? "http://localhost:8080";

/**
 * Cookie policy. `Secure` + the `__Host-` name prefix are relaxed ONLY for
 * plain-http loopback; see `./cookie-policy` for why that is required for local
 * sign-in, and why production/LAN/tunnel origins are unaffected.
 */
export const secureCookies = shouldUseSecureCookies(localOrigin);

/** Session cookie name — also read by the live-preview popup completion page. */
export const SESSION_TOKEN_COOKIE = sessionTokenCookieName(localOrigin);

/** Same policy for the remaining Better Auth cookies. */
const sessionDataCookieName = sessionDataCookieNameFor(localOrigin);
const accountDataCookieName = accountDataCookieNameFor(localOrigin);
const dontRememberCookieName = dontRememberCookieNameFor(localOrigin);
...
export const auth = betterAuth({
  ...
  // `disableSignUp` keeps the admin account server-provisioned: only the seed in
  // `@/lib/admin-auth.server` can create it, so nobody can register their way in.
  ...
  advanced: {
    // `useSecureCookies: false` means Better Auth does not apply its own Secure
    // prefixing — we set Secure ourselves via `defaultCookieAttributes` below,
    // except on plain-http loopback where the browser would discard the cookie.
    useSecureCookies: false,
    defaultCookieAttributes: { secure: secureCookies, sameSite: "lax", path: "/" },
    cookies: {
      session_token: { name: SESSION_TOKEN_COOKIE },
      session_data: { name: sessionDataCookieName },
      account_data: { name: accountDataCookieName },
      dont_remember: { name: dontRememberCookieName },
    },
  },
